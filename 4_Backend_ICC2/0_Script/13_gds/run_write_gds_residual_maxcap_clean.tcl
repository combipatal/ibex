################################################################################
# Ibex post-route clean-block GDS candidate export.
#
# This writes an educational GDS candidate from a selected post-route clean
# ICC2 block. Defaults target the residual max-cap ECO block, but SOURCE_*,
# SRC_BLOCK, and GDS_TAG can select later clean candidates such as the
# pre-filler max-cap margin ECO. It is not a tapeout/signoff GDS.
################################################################################

source 4_Backend_ICC2/0_Script/00_setup/icc2_common_setup.tcl

set SRC_ICC2_LIB $PROJECT_ROOT/4_Backend_ICC2/2_Output/13_gds/post_route_residual_maxcap_eco_gds_candidate/ibex_mini_soc_top_post_route_residual_maxcap_eco_gds_candidate_icc2_lib
set SOURCE_CLEAN_ICC2_LIB $PROJECT_ROOT/4_Backend_ICC2/2_Output/12_post_route_residual_maxcap_eco/ibex_mini_soc_top_post_route_residual_maxcap_eco_icc2_lib
set SRC_BLOCK ${TOP_NAME}_post_route_residual_maxcap_eco
set GDS_TAG post_route_residual_maxcap_eco_gds_candidate

if {[info exists ::env(SRC_ICC2_LIB)] && $::env(SRC_ICC2_LIB) ne ""} {
  set SRC_ICC2_LIB $::env(SRC_ICC2_LIB)
}
if {[info exists ::env(SOURCE_CLEAN_ICC2_LIB)] && $::env(SOURCE_CLEAN_ICC2_LIB) ne ""} {
  set SOURCE_CLEAN_ICC2_LIB $::env(SOURCE_CLEAN_ICC2_LIB)
}
...
set OUTPUT_DIR $PROJECT_ROOT/4_Backend_ICC2/2_Output/13_gds/$GDS_TAG
set REPORT_DIR $PROJECT_ROOT/4_Backend_ICC2/4_Report/13_gds/$GDS_TAG
set LOG_DIR $PROJECT_ROOT/4_Backend_ICC2/3_Log/13_gds
...
if {![file exists $SRC_ICC2_LIB]} {
  error "Missing copied GDS ICC2 library: $SRC_ICC2_LIB"
}
...
report_constraints -all_violators > $REPORT_DIR/constraints.before_filler.rpt
...
puts $FP "source_clean_icc2_lib=$SOURCE_CLEAN_ICC2_LIB"
puts $FP "gds_icc2_lib=$SRC_ICC2_LIB"
