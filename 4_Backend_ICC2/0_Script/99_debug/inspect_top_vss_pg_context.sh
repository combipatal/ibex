...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/inspect_top_vss_pg_context.log"

icc2_shell -f 4_Backend_ICC2/0_Script/99_debug/inspect_top_vss_pg_context.tcl \
  -output_log_file "$LOG_FILE"

if rg -n "^(Error:|ERROR:)|stopped at line .* due to error" "$LOG_FILE"; then
  echo "Top VSS PG context inspection finished with errors. See $LOG_FILE" >&2
  exit 1
fi

echo "Top VSS PG context inspection finished. Log: $LOG_FILE"
