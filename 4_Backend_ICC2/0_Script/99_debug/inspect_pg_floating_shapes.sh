...
mkdir -p "$LOG_DIR"
LOG_FILE="$LOG_DIR/inspect_pg_floating_shapes.log"

icc2_shell -f 4_Backend_ICC2/0_Script/99_debug/inspect_pg_floating_shapes.tcl \
  -output_log_file "$LOG_FILE"

if rg -n "^(Error:|ERROR:)|stopped at line .* due to error" "$LOG_FILE"; then
  echo "PG floating shape inspection finished with errors. See $LOG_FILE" >&2
  exit 1
fi

echo "PG floating shape inspection finished. Log: $LOG_FILE"
